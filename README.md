...
纯文件消息的上传信息和保存回执会先持久化到业务数据库。下一次模型调用前，系统按当前 Session 和 Turn 顺序补入最近 20 条此类上下文事件；已经存在于活动上下文的事件不重复插入，压缩后缺失时可重新补入。Trace 中以 `context_replay` 展示补入内容。这样上传后直接说“看一下这个简历”即可引用刚上传的文件，不必手动再报文件 ID。图片继续通过 Pi 的图片消息进入历史。
