...
**当前边界**：本地代码默认支持 PNG/JPEG 图片回传，真实微信送达待验收；不回传其他文件。原始 PDF 输入只接入官方 Codex 后端；记忆搜索不使用 embedding。Pi 的其他模型通道依赖其认证与模型配置，不代表都已接入文件输入。

## 图片回复与截图

服务加载新代码后，Agent 默认可调用 `reply_image(path)` 保存候选图片，Task 审核通过后由 Outbox 上传和发送。截图复用现有 Playwright / 系统命令，项目内置 `screenshot-reply` Skill（沿用现有 Pi Skill 加载配置），不新增截图专用原生工具。

仅支持 PNG/JPEG，每张最多 10 MiB、2500 万像素，每次回复最多 3 张。工具登记、审核完成与微信送达是不同状态；图片网络重试不重新截图。当前已验证空白网页真实截图及本地链路，实际宿主桌面权限和真实微信可打开图片仍待验收。[规格](docs/specs/image-delivery/spec.md) · [启用与排障](docs/runbook/image-replies.md)
