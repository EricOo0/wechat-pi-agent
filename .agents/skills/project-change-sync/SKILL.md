...
先执行 project-spec-review，再根据已确认结论同步文档。发现遗漏/偏差时修实现或提出规格变更，不先把规格改成现有实现。
...
Hook 模式中 spec 正文和 reviews 已由前置核对冻结；docs/specs 下只可建议更新 MAP.md。规格要求如需变化，返回 blocker，由当前开发任务明确后重新核对。不得改写前置核对报告或自动消除偏差。
